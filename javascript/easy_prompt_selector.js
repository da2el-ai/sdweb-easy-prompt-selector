class EPSElementBuilder {
  ...
  static openButton({ onClick }) {
    const button = EPSElementBuilder.baseButton("🔯タグを選択", {
      size: "sm",
      color: "secondary",
    });
...
class EasyPromptSelector {
  ...
  async parseFiles() {
    ...
    // タグファイルをjsonで取得
    const response = await fetch(`/eps/tags?${new Date().getTime()}`);
      ...
      Object.keys(this.tags),
      {
        onChange: (selected) => {
          ...
          Array.from(content.childNodes).forEach((node) => {
            const visible =
              node.id === `easy-prompt-selector-container-${selected}`;
...
const easyPromptSelector = new EasyPromptSelector(gradioApp());
...
onUiLoaded(async () => {
  ...
  const button = EPSElementBuilder.openButton({
    onClick: () => {
      const tagArea = gradioApp().querySelector(
        `#${easyPromptSelector.AREA_ID}`
      );
  ...
  const reloadButton = gradioApp().getElementById(
    "easy_prompt_selector_reload_button"
  );
